...
verify_full(void)
{
  ...
  // Precision of integers. For compatibility reasons this is not
  // contained in PrioConfig and must be set on client- and
  // serverside.
  const int prec = 32;
  ...
  // Maximum integer for prec
  long max = (1l << (prec)) - 1;

  // Generate client data packets.
  for (int c = 0; c < nclients; c++) {

    // The client's data submission is an arbitrary b-bit integer vector.
...
main(void)
{
  puts("This utility demonstrates how to invoke the Prio API for b-bit integer "
       "vectors.");
  return verify_full();
}
