...
mu_test_client__new(void)
{
  ...
  PT_CHECKA(cfg = PrioConfig_newTest(23, 1));
...
void
test_client_agg(int nclients, int nfields, int precision, bool config_is_okay)
{
  SECStatus rv = SECSuccess;
  PublicKey pkA = NULL;
  PublicKey pkB = NULL;
  ...
  P_CHECKA(
    cfg = PrioConfig_new(nfields, precision, pkA, pkB, batch_id, batch_id_len));
  if (!config_is_okay) {
    PT_CHECKCB(
      (PrioConfig_new(nfields, precision, pkA, pkB, batch_id, batch_id_len) ==
       NULL));
  }
...
{
  test_client_agg(1, 133, 32, true);
}

void
mu_test_client__agg_2(void)
{
  test_client_agg(2, 133, 32, true);
}

void
mu_test_client__agg_10(void)
{
  test_client_agg(10, 133, 32, true);
}
...
{
  int prec = 32;
  int max = MIN(PrioConfig_maxDataFields(prec), 500);
  test_client_agg(10, max, prec, true);
}
...
{
  int prec = 32;
  int max = PrioConfig_maxDataFields(prec);
  test_client_agg(10, max + 1, prec, false);
}
